...
function paginated_gallery($attr) {
	...
	*/
	...
	if ($pageCount > 1)
	{
		...
		$gplist= '<div id="paginacao"><ul class="page-numbers">';
		for ( $j=1; $j<= $pageCount; $j++)
		{
			$gplist .= '<li>';

			if ( $j==$currentPage )
				$gplist .= '<span class="page-numbers current"> '.$j.' </strong>';
			else
				$gplist .= '<a class="page-numbers" href="'.$page_link. ( ($page_link_perma?'?':'&amp;') ). 'galleryPage='.$j.'">'.$j.'</a>';
